...
export function TopNavbar({ onMenuClick }) {
	...
	return (
		<header
			className="sticky top-0 z-30 flex h-16 shrink-0 items-center border-b border-[var(--color-border)] px-4 backdrop-blur-xl sm:px-6"
		...
		>
			...
			<div className="flex min-w-0 flex-1 items-center gap-3">
				...
				<button
					...
					className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-[var(--color-muted)] transition-colors hover:bg-[var(--color-surface-2)] hover:text-[var(--color-text)] md:hidden"
				>
				...
				<div className="min-w-0">
					...
					{meta.subtitle && (
						<p className="hidden truncate text-[12px] text-[var(--color-muted)] sm:block">
							{meta.subtitle}
						</p>
					)}
				</div>
